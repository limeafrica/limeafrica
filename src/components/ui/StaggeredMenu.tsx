...
export const StaggeredMenu = forwardRef<StaggeredMenuHandle, StaggeredMenuProps>(function StaggeredMenu(
  {
    ...
    className,
    logoUrl = "/lime-logo-2024-1-png.avif",
    menuButtonColor = "#fff",
  ...
  useLayoutEffect(() => {
    ...
    return () => ctx.revert();
  }, [hideHeader, menuButtonColor, position]);
  ...
  useEffect(() => {
    ...
    function handleClickOutside(event: MouseEvent) {
      ...
      const insideToggle =
        Boolean(toggleBtnRef.current?.contains(t)) ||
        Boolean(externalToggleRef?.current?.contains(t));
      if (panelRef.current && !panelRef.current.contains(t) && !insideToggle) {
        closeMenu();
      }
    }
  ...
  }, [closeOnClickAway, externalToggleRef, open, closeMenu]);

  const accentStyle = accentColor ? ({ ["--sm-accent" as string]: accentColor } as React.CSSProperties) : undefined;
      ...
      {!hideHeader ? (
        <header className="staggered-menu-header" aria-label="Main navigation header">
          <div className="sm-logo" aria-label="Logo">
            <img
              src={logoUrl || "/lime-logo-2024-1-png.avif"}
              alt=""
          ...
          <ul className="sm-panel-list" role="list" data-numbering={displayItemNumbering || undefined}>
            {items && items.length ? (
              items.map((it, idx) => {
                ...
                const dataIdx = { "data-index": idx + 1 } as const;

                return (
                  <li className="sm-panel-itemWrap" key={it.label + String(idx)}>
                    {isInternalHref(it.link) ? (
                      <Link
                        href={it.link}
